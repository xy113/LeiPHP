...
// JavaScript Document
var DSXUpload = function(settings){
	var that = this;
	var opts = $.extend({
		debug:false,
		queue_id : '',
	...
	var handlers = {
		...
		onDialogOpen : function(){
			if(opts.onDialogOpen) opts.onDialogOpen();
		},
		onDialogComplete : function(filesSelected, filesQueued, queueLength){
			if(filesQueued > 0){
				if(opts.autoStart) this.startUpload();
			}
			if(opts.onDialogComplete) opts.onDialogComplete(filesSelected, filesQueued, queueLength);
		},
		onSelect : function(file){
			...
			if(opts.onSelect) opts.onSelect(file);
		},
			...
			*/
			switch(errorCode) {
				...
				case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
					alert('文件"'+file.name+'"超过了限制');
					break;
				case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
					alert('文件"'+file.name+'"大小为0，不能上传');
					break;
				case SWFUpload.QUEUE_ERROR.QUEUE_ERROR.INVALID_FILETYPE:
					alert('文件"'+file.name+'"类型错误');
					break;
			}
			if(opts.onSelectError) opts.onSelectError(file, errorCode, errorMsg);
		},
		...
		onProgress : function(file, bytesLoaded, bytesTotal){
			...
			var lapsedTime  = newTime - that.timer;
			if (lapsedTime > 500) {
				that.timer = newTime;
			}
			var lapsedBytes    = bytesLoaded - that.bytesLoaded;
			that.bytesLoaded  = bytesLoaded;
			...
			if (that.averageSpeed > 0) {
				that.averageSpeed = Math.floor((that.averageSpeed + kbs) / 2);
			} else {
				that.averageSpeed = Math.floor(kbs);
			}
			if (kbs > 1000) {
				mbs = (kbs * .001);
				that.averageSpeed = Math.floor(mbs);
				suffix = 'MB/s';
			}
			that.averageSpeed+= suffix
			if(opts.onProgress) opts.onProgress(file, bytesLoaded, bytesTotal, percentage, that.averageSpeed);
		},
		onUploadError : function(file, errorCode, errorMsg){
			if(opts.onUploadError) opts.onUploadError(file, errorCode, errorMsg);
		},
		onUploadSuccess : function(file, data, response){
			if(opts.onUploadSuccess) opts.onUploadSuccess(file, data, response);
		},
		onQueueComplete : function(file){
			if(opts.onQueueComplete) opts.onQueueComplete(file);
		},
		onDebug : function(){
	...
	this.init = function(){
		that.uploader = new SWFUpload({
			...
			button_image_url : opts.button_image,
			...
			button_action : opts.multi ? SWFUpload.BUTTON_ACTION.SELECT_FILES : SWFUpload.BUTTON_ACTION.SELECT_FILE,
			button_disabled : opts.button_disabled,
			button_cursor : SWFUpload.CURSOR.HAND,
			button_window_mode : SWFUpload.WINDOW_MODE.TRANSPARENT,
			
	 
			swfupload_loaded_handler : handlers.onReady,
			file_dialog_start_handler : handlers.onDialogOpen,
			file_dialog_complete_handler : handlers.onDialogComplete,
			file_queued_handler : handlers.onSelect,
			file_queue_error_handler : handlers.onSelectError,
			upload_start_handler : handlers.onStart,
			upload_progress_handler : handlers.onProgress,
			upload_error_handler : handlers.onUploadError,
			upload_success_handler : handlers.onUploadSuccess,
			upload_complete_handler : handlers.onQueueComplete,
			debug_handler : handlers.onDebug
		});
		var wrap = $('<div>').attr('id',opts.button_id+'-button').addClass(opts.button_class);
		$('#'+that.uploader.movieName).wrap(wrap);
	}
	
	this.startUpload = function(fileID){
		if(fileID){
			that.uploader.startUpload(fileID);
		}else {
	...
	this.cancelUpload = function(fileID, triggerErrorEvent){
		that.uploader.cancelUpload(fileID, triggerErrorEvent);
		if(opts.onCancel) opts.onCancel(fileID);
	}
